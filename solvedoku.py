...
class Board:
    def __init__(self, arr: list) -> None:
        ...
        if not isinstance(arr, list):
            ...
            correct_dims = False
        if not correct_dims:
            raise TypeError("The given value for 'arr' is not a 9x9 list of integers.")
            ...
            for idy, col in enumerate(row):
                ...
                if col is None:
                    for idp, p in enumerate(curr_poss[idx][idy]):
                        if not self.row[idx][p] and not self.col[idy][p] and not self.block[self.__get_block_num(idx, idy)][p]:
